...
<div class="bg-white p-6 md:p-8 rounded-2xl shadow-lg max-w-2xl mx-auto">
    <div class="mb-6">
        <h1 class="text-2xl md:text-3xl font-bold text-gray-800">Tambah Lead Baru</h1>
        <p class="text-gray-500 mt-1">Isi detail di bawah ini untuk membuat lead baru.</p>
    </div>
    ...
    <form method="POST" action="{{ route('leads.store') }}" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" id="phone" name="phone" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary focus:border-primary block w-full p-2.5" value="{{ old('phone') }}">
            </div>
        </div>
        <div>
            <label for="email" class="block mb-2 text-sm font-medium text-gray-700">Alamat Email</label>
            <input type="email" id="email" name="email" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary focus:border-primary block w-full p-2.5" value="{{ old('email') }}">
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
        ...
        <div class="flex justify-end pt-4">
            ...
            <button type="submit" class="px-6 py-2.5 bg-primary-dark text-white font-semibold text-sm rounded-lg shadow-md hover:bg-pink-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary-dark transition">Simpan Lead</button>
        </div>
    </form>
</div>
@endsection
